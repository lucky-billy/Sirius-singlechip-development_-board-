...
{		
	...
	GPIO_PinAFConfig(GPIOF, GPIO_PinSource6, GPIO_AF_TIM10); 	// GPIOF6复用为TIM10
	
	TIM_TimeBaseStructure.TIM_Prescaler=167;  				// 预分频器,1M的计数频率,1us加1.	
	TIM_TimeBaseStructure.TIM_CounterMode=TIM_CounterMode_Up;	// 向上计数模式
	TIM_TimeBaseStructure.TIM_Period=0XFFFFFFFF;   			// 设定计数器自动重装值 最大10ms溢出  
	...
	TIM_ICInitStructure.TIM_ICFilter = 0x03;				// IC1F=0003 8个定时器时钟周期滤波
	TIM_ICInit(TIM10, &TIM_ICInitStructure);				// 初始化定时器2输入捕获通道
...
{ 		
	if(TIM_GetITStatus(TIM10,TIM_IT_Update)!=RESET) 	// 溢出中断
	{
	...
	if(TIM_GetITStatus(TIM10,TIM_IT_CC1)!=RESET) 		// 处理捕获(CC1IE)中断
	{	  
		if(RDATA)	// 上升沿捕获
		{
		...
		{
			Dval=TIM_GetCapture1(TIM10);// 读取CCR1也可以清CC1IF标志位
	...
	TIM_ClearFlag(TIM10,TIM_IT_Update|TIM_IT_CC1);
}
